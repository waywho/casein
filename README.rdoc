...
**WORK IN PROGRESS ON CASEIN 5.0.0.beta1 — featuring new responsive UI based on Bootstrap 3**
...
— Add the Casein gem to your Gemfile: (temporary git link, until official release)
...
— Run the following Rake task to set up an initial user. The default username and password will be displayed once the task has completed. You should specify your email address. If you set up an SMTP server in your Rails environment then you’ll also receive an email notification about the new account.
